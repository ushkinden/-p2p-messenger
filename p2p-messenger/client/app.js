(() => {
  ...
  // Вторая, отдельная от подписи, пара ключей — для сквозного шифрования
  // сообщений, которые сервер временно держит у себя для офлайн-получателя.
  // Приватный ключ так же непригоден к экспорту.
  async function generateEncryptionKeyPair() {
    return crypto.subtle.generateKey(ENC_ALG, false, ['deriveKey']);
  }
  async function importPeerEncPublicKey(b64) {
    return crypto.subtle.importKey('spki', bytesFromB64(b64), ENC_ALG, false, []);
  }
  // Оба участника, применив ECDH к своему приватному и чужому публичному
  // ключу, получают один и тот же общий ключ — им и шифруется сообщение
  // для очереди на сервере. Сервер этого ключа никогда не видит.
  ...
  async function loadOrCreateIdentity() {
    ...
    if (existing && existing.keyPair && existing.encKeyPair) {
      return { keyPair: existing.keyPair, encKeyPair: existing.encKeyPair };
    }
    // У ранее созданной личности может уже быть ключ подписи, но не быть
    // ключа шифрования (добавлен позже) — донастраиваем, не трогая старый.
    const keyPair = existing && existing.keyPair ? existing.keyPair : await generateIdentityKeyPair();
    const encKeyPair = existing && existing.encKeyPair ? existing.encKeyPair : await generateEncryptionKeyPair();
    await idbPut('identity', { key: 'me', keyPair, encKeyPair });
    return { keyPair, encKeyPair };
  }
  ...
  const state = {
    ...
    myPublicKeyB64: null,
    encKeyPair: null, // пара для сквозного шифрования сообщений на сервере
    myEncPublicKeyB64: null,
  ...
  async function afterUnlock() {
    ...
    state.identityKeyPair = identity.keyPair;
    state.encKeyPair = identity.encKeyPair;
    state.myPublicKeyB64 = await exportPublicKeyB64(state.identityKeyPair.publicKey);
    state.myEncPublicKeyB64 = await exportPublicKeyB64(state.encKeyPair.publicKey);
    if (needsSetup()) {
  ...
  // Возвращает { publicKey, encPublicKey } собеседника или null, если
  // сервер их не знает (собеседник ни разу не выходил в сеть) либо не
  // ответил вовремя.
  ...
  // Подгружает и сохраняет ключ шифрования контакта, если его ещё нет —
  // без него сообщения этому контакту нельзя поставить в серверную очередь.
  async function ensureContactEncKey(contact) {
    if (contact.encPublicKey) return contact.encPublicKey;
    const keys = await requestPubkey(contact.id);
    if (keys && keys.encPublicKey) {
      contact.encPublicKey = keys.encPublicKey;
      await idbPut('contacts', contact);
    }
    return contact.encPublicKey || null;
  }
  ...
  async function handleSignalingMessage(msg) {
    switch (msg.type) {
      case 'challenge': {
        ...
        send({ type: 'register', id: state.myId, publicKey: state.myPublicKeyB64, encPublicKey: state.myEncPublicKeyB64, signature });
        break;
      }
      case 'registered':
      ...
      case 'presence-update': {
        ...
        if (msg.online) {
          ...
          if (contact && (!contact.publicKey || !contact.encPublicKey)) {
            requestPubkey(msg.id).then(async (keys) => {
              if (!keys) return;
              if (keys.publicKey) contact.publicKey = keys.publicKey;
              if (keys.encPublicKey) contact.encPublicKey = keys.encPublicKey;
              await idbPut('contacts', contact);
            });
          }
        }
      ...
      case 'pubkey': {
        ...
        if (resolve) {
          ...
          resolve(msg.publicKey ? { publicKey: msg.publicKey, encPublicKey: msg.encPublicKey || null } : null);
        }
        break;
      }
      case 'queued_message':
        await handleIncomingQueuedMessage(msg);
        break;
      case 'signal':
  ...
  el('add-contact-confirm').addEventListener('click', async () => {
    ...
    const keys = await requestPubkey(id);
    const contact = {
      id, name: name || formatId(id), addedAt: Date.now(),
      publicKey: keys?.publicKey || null,
      encPublicKey: keys?.encPublicKey || null,
    };
    await idbPut('contacts', contact);
  ...
  el('safety-code-btn').addEventListener('click', async () => {
    ...
    if (!peerKey) {
      ...
      if (keys?.publicKey) {
        ...
        contact.publicKey = keys.publicKey;
        if (keys.encPublicKey) contact.encPublicKey = keys.encPublicKey;
        await idbPut('contacts', contact);
      }
    }
  ...
  function renderMessage(m) {
    ...
    if (m.direction === 'out') {
      ...
      status.textContent = m.status === 'pending' ? '· ожидает' : m.status === 'queued' ? '· на сервере' : m.status === 'sent' ? '· отправлено' : '';
  ...
  async function attemptDeliver(peerId) {
    ...
    if (!peerConn.dc || peerConn.dc.readyState !== 'open') {
      ...
      else await queueToServerIfOffline(peerId);
      return;
    }
  ...
  function updateBubbleStatus(m) {
    ...
    if (statusEl) statusEl.textContent = m.status === 'queued' ? '· на сервере' : '· отправлено';
  }

  // Собеседник сейчас не в сети совсем (не просто устанавливается прямое
  // соединение) — шифруем ожидающие сообщения его ключом и оставляем на
  // сервере; он заберёт их и они сразу же удалятся, как только подключится.
  const SERVER_QUEUE_MAX_BLOB_BYTES = 4 * 1024 * 1024;
  async function queueToServerIfOffline(peerId) {
    const contact = state.contacts.find((c) => c.id === peerId);
    if (!contact) return;
    const encKeyB64 = await ensureContactEncKey(contact);
    if (!encKeyB64) return; // собеседник ни разу не выходил в сеть — ждём прямого соединения
    const pending = (await idbGetByIndex('messages', 'peerId', peerId))
      .filter((m) => m.direction === 'out' && m.status === 'pending')
      .sort((a, b) => a.createdAt - b.createdAt);
    if (!pending.length) return;
    const peerEncKey = await importPeerEncPublicKey(encKeyB64);
    const sharedKey = await deriveSharedAesKey(state.encKeyPair.privateKey, peerEncKey);
    for (const m of pending) {
      try {
        let plainBytes;
        const extra = {};
        if (m.kind === 'text') {
          plainBytes = new TextEncoder().encode(await decryptText(m));
        } else {
          const blob = await decryptBlob(m, m.mimeType);
          if (blob.size > SERVER_QUEUE_MAX_BLOB_BYTES) continue; // большой файл ждёт прямого соединения
          plainBytes = new Uint8Array(await blob.arrayBuffer());
          extra.fileName = m.fileName;
          extra.mimeType = m.mimeType;
          extra.fileSize = m.fileSize;
        }
        const enc = await aesEncrypt(sharedKey, plainBytes);
        send({
          type: 'queue_message', to: peerId, msgId: m.msgId, kind: m.kind,
          encIv: bytesToB64(enc.iv), encData: bytesToB64(enc.ciphertext),
          ts: m.createdAt, ...extra,
        });
        m.status = 'queued';
        await idbPut('messages', m);
        if (state.activePeerId === peerId) updateBubbleStatus(m);
      } catch (e) {
        console.error('Не удалось поставить сообщение в очередь на сервере:', e);
      }
    }
  }

  // Сообщение, которое сервер придержал для нас, пока мы были офлайн —
  // расшифровываем общим с отправителем ключом и сохраняем как обычное
  // входящее; сервер своё уже отдал и удалил.
  async function handleIncomingQueuedMessage(msg) {
    if (!state.contacts.some((c) => c.id === msg.from)) return; // не наш контакт — игнорируем
    const contact = state.contacts.find((c) => c.id === msg.from);
    const encKeyB64 = await ensureContactEncKey(contact);
    if (!encKeyB64) {
      console.warn('Нет ключа шифрования отправителя — сообщение с сервера потеряно.');
      return;
    }
    let plainBytes;
    try {
      const senderEncKey = await importPeerEncPublicKey(encKeyB64);
      const sharedKey = await deriveSharedAesKey(state.encKeyPair.privateKey, senderEncKey);
      plainBytes = await aesDecrypt(sharedKey, bytesFromB64(msg.encIv), bytesFromB64(msg.encData));
    } catch (e) {
      console.error('Не удалось расшифровать сообщение с сервера:', e);
      return;
    }
    let localEnc, plainText, plainBlob;
    if (msg.kind === 'text') {
      plainText = new TextDecoder().decode(plainBytes);
      localEnc = await encryptText(plainText);
    } else {
      plainBlob = new Blob([plainBytes], { type: msg.mimeType || 'application/octet-stream' });
      localEnc = await encryptBlob(plainBlob);
    }
    const m = {
      msgId: msg.msgId, peerId: msg.from, direction: 'in', kind: msg.kind,
      encIv: localEnc.iv, encData: localEnc.ciphertext,
      fileName: msg.fileName, fileSize: msg.fileSize, mimeType: msg.mimeType,
      createdAt: msg.ts || Date.now(), status: 'received',
    };
    await idbPut('messages', m);
    if (state.activePeerId === msg.from) {
      renderMessage(msg.kind === 'text' ? { ...m, text: plainText } : { ...m, blob: plainBlob });
      scrollToBottom();
    }
    notifyNewMessage(msg.from, msg.kind === 'text' ? plainText : msg.kind === 'image' ? '📷 Фото' : msg.kind === 'video' ? '🎞 Видео' : `📎 ${m.fileName || 'файл'}`);
  }
